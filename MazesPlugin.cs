...
namespace MazesPlugin
{
	...
	public class MazesPlugin : TerrariaPlugin
	{
        ...
        void OnInitialize(EventArgs e)
        {
            ...
            TShockAPI.Commands.ChatCommands.Add(new Command("mazes", Point1, "/mpoint1"));
            TShockAPI.Commands.ChatCommands.Add(new Command("mazes", Point2, "/mpoint2"));
		...
		void Inset(CommandArgs e)
		{
			...
			e.Player.SendInfoMessage(String.Format("Inset selection by {0}.", amount));
		}
		...
		void Outset(CommandArgs e)
		{
			...
			e.Player.SendInfoMessage(String.Format("Outset selection by {0}.", amount));
		}
        ...
        void Redo(CommandArgs e)
		{
			if (e.Parameters.Count > 1)
			{
				e.Player.SendErrorMessage("Invalid syntax! Proper syntax: //redo [steps]", Color.Red);
				return;
			}
			...
			{
				e.Player.SendErrorMessage("Invalid number of steps.", Color.Red);
				return;
			}
        ...
        void Size(CommandArgs e)
        {
            ...
            if (info.x == -1 || info.y == -1 || info.x2 == -1 || info.y2 == -1)
            {
                e.Player.SendErrorMessage("Invalid selection.", Color.Red);
                return;
            }
            int lenX = Math.Abs(info.x - info.x2) + 1;
            int lenY = Math.Abs(info.y - info.y2) + 1;
            e.Player.SendInfoMessage(String.Format("Selection size: {0} x {1}", lenX, lenY), Color.Yellow);
        }
		void Undo(CommandArgs e)
		{
			if (e.Parameters.Count > 1)
			{
				e.Player.SendErrorMessage("Invalid syntax! Proper syntax: //undo [steps]", Color.Red);
				return;
			}
